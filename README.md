...
- transport 非依存の index と API 参照面
...
raw な transport event は正本ではありません。
正本は canonical な knowledge object です。

WebSocket リレー、HTTP gateway、ファイル archive、将来の federated carrier のような transport は、プロトコルの外側にある実装選択です。
