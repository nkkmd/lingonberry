...
Phase 0 で固定する core fields は次の通りです。
...
- inquiry object の必須フィールド
- inquiry から observation / evidence / synthesis への relation
...
1. delete は tombstone だけにするか、限定操作として許すか
2. private または encrypted object を初期版に含めるか
3. relation を triple、edge、statement object のどれで表すか
4. identity claim を必須にするか任意にするか
5. relay は最小限の semantic validation を行うか、protocol object schema / carrier framing validation のみにするか
6. 最初の正規 carrier は relay-based pub/sub、HTTP publish、file/archive ingest のどれにするか
7. carrier capability を中央 registry で管理するか、完全分散にするか
8. application profile を中央 registry で管理するか、完全分散にするか
9. 多言語の正本をどう表現するか
10. public relay が object を受け入れるための最小 trust model は何か
11. Toitoi profile の最小必須 object / relation set はどこまでにするか
