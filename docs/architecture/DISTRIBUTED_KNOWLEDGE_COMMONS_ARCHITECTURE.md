...
- transport の選択を semantic core の外に置くこと
- Toitoi のような特定アプリケーションが、この基盤の上に乗れても、プロトコル自体はその分野に縛られないこと

要するに、

> プロトコルは知識循環の意味を定義し、
> transport はその意味をどう運ぶかを定義します。
...
transport の識別子は、ルーティングと重複排除のために使います。
canonical 識別子は、commons 内の意味的 identity に使います。

### 2.4 replay 可能性

どのノードも、raw archive から canonical state を再構築できる必要があります。

そのためには、次が必要です。

- raw event の保持
- 決定的な canonicalization
- versioned な schema 規則
- 明示的な migration 経路

### 2.5 transport 非依存性

このプロトコルは、複数の carrier 上で使える必要があります。
...
- HTTP publish API
- file/archive ingest
- 将来の federated sync や offline sync transport

transport 固有の規則は adapter と relay に閉じ、semantic model には持ち込みません。
...
この層は、raw な transport object を保存し、転送します。

責務:

- publish されたオブジェクトを受け取る
- transport-level の構造を検証する
- 必要に応じて transport 署名を認証・検証する
- transport identity で重複排除する
...
この層は、raw transport object を canonical な knowledge object に変換します。
...
API は、明示的に求められない限り transport 固有の詳細を隠すべきです。
...
| `provenance` | 来歴と変換履歴 |
| `rawRef` | transport/raw payload への参照 |
...
Transport identity は、特定の carrier が使う識別子です。
...
Transport identity は routing、duplicate check、source lookup に使います。
...
- transport をまたいでも、同じ semantic object を指すなら安定している
- transport semantics に強く依存しない opaque な値である
...
Identity key は、canonicalization 規則から決定的に導出される transport-neutral な値です。

用途:

- transport 間で object を比較する
- canonical identity を解決する
...
Raw reference は、元の transport object または payload を指します。
...
Raw reference は provenance とは分けて保持します。

## 6. Transport と relay のモデル
...
- raw object を受け取る
- transport-level の整合性を検証する
- append-only の raw log を保存する
...
- **Gateway relay**: transport 間の変換を担う
...
Canonicalization パイプラインは、決定的であるべきです。
...
- raw transport payload を主インターフェースとして見せること
- transport 固有の実装詳細
- ingest 内部の不安定な表現
...
- `packages/adapters/`
  - transport から canonical への変換
- `packages/transports/`
  - transport 固有の schema と helper
- `packages/relay/`
...
6. relay は最小限の semantic validation を行うか、transport validation のみにするか
7. 最初の transport は relay-based pub/sub、HTTP publish、file/archive ingest のどれにするか
