...
## フェーズ 3: identity と provenance を実用化する（完了済み）

### 目的

carrier をまたいでも同じ semantic object を追跡できるようにします。

Phase 3 は、identity key の導出、identity claim の生成・検証、provenance の保存・取得、rawRef からの再取得まで完了しています。

### 最初の着手順

Phase 3 では、次の順で切り出すと進めやすいです。

1. `identity key` の導出規則を固定する
2. `identity claim` の生成と検証を実装する
3. `provenance` の保存と取得を実装する
4. `rawRef` からの再取得経路を実装する

この順にすると、carrier をまたぐ同一性の追跡と、来歴の保存・再構成を分けて進められます。
...
Phase 4 は、次の順で切り出すと進めやすいです。

1. type index を作る
2. relation graph を作る
3. lineage graph を作る
4. provenance graph を作る
5. canonical view API の read-only 面を固める
6. index 再構築手順を固定する

まずは canonical store を直接走査して、派生 index を作れることを確認します。  
その後で API を薄く載せ、index 破損と semantic data 破損を分離します。
