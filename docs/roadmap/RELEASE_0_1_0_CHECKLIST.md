...
- ファイル名、配置、リンク切れの最終確認は、次の段階で個別に点検する
...
- `schemas/README` と各 schema ファイルの説明の対応は、別途 schema 側の詳細確認が必要なので、ここでは未完了のまま残す
...
- `lingonberry-storage capabilities`、`config`、`ready` は実行できた
- `serve-http`、`publish`、`export-archive`、`import-archive`、`run`、fixture の validate / reject は、まだ実行していないので未完了のまま残す
...
- release note、source archive、タグ名の一致は、まだ実物または別文書の確認が必要なので未完了のまま残す
...
- `0.1.0` のサポート外項目と、失敗時の参照文書の一本化は、まだ明示の確認を増やしたくないので未完了のまま残す
