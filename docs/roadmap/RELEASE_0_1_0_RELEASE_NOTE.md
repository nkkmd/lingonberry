...
Lingonberry `v0.1.0` は、core protocol、application profile の境界、HTTP carrier、file / archive carrier、schemas、fixtures、最小 runtime 実装、運用メモを source release として公開する最初の版です。
...
- `v0.1.0` tag と release title が一致していること
...
- federated carrier は将来の拡張として扱い、v0.1.0 では正規 carrier の基礎に集中する
- relay / storage / archive の運用確認を引き続き固める
- carrier 拡張時の fail closed と failure routing を runbook に寄せる

## post-release follow-up

v0.1.0 の公開後に直したい事項は、次の backlog に分けて追跡します。
...
タグ名を `v0.1.0` にする場合の例は次のとおりです。

```bash
git archive --format=tar.gz --prefix=lingonberry-v0.1.0/ v0.1.0 > lingonberry-v0.1.0.tar.gz
```
...
- [v0.1.0 公開前チェックリスト](./RELEASE_0_1_0_CHECKLIST.md)
- [ロードマップ](./README.md)
