...
Status: **substantially completed; backend-internal cleanup remains**
...
- [ ] File backend内部に残る防御的な手書き判定を共通classifierへ置換
- [ ] SQLite backend内部に残る防御的な手書き判定を共通classifierへ置換
...
- [ ] `Cargo.lock`同期
- [x] Rust format／Clippy／workspace tests（Phase 1／Phase 2 classified paths）
- [x] JavaScript contract tests（Phase 1／Phase 2 classified paths）
...
- #79: duplicate／conflict public contract and classified live path（completed）
- #80: storage／maintenance classified path consolidation（in progress）
