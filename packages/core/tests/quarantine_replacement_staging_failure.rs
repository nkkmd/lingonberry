...
use lingonberry_core::{
    ...
    QuarantineReplacementTransactionState, QUARANTINE_REPLACEMENT_STAGING_DIR,
};
...
const STAGED_LEDGER_WRITE_FAILURE: &str = "staging.ledger-write";
const STAGED_LEDGER_FSYNC_FAILURE: &str = "staging.ledger-fsync";
const STAGING_DIRECTORY_FSYNC_FAILURE: &str = "staging.directory-fsync";
...
fn fixture() -> (PathBuf, PathBuf, PathBuf, PathBuf) {
    let state = temp_dir("staging-failure-state");
    let backup = temp_dir("staging-failure-backup");
    let proof = temp_dir("staging-failure-proof");
    let transaction = temp_dir("staging-failure-transaction");
...
#[test]
fn injected_staging_failures_clean_partial_state_and_retry_to_committed() {
    ...
    for (point, transaction_id, expected_state) in [
        (
            STAGED_LEDGER_WRITE_FAILURE,
            "tx-staging-ledger-write",
            QuarantineReplacementTransactionState::Prepared,
        ),
        (
            STAGED_LEDGER_FSYNC_FAILURE,
            "tx-staging-ledger-fsync",
            QuarantineReplacementTransactionState::RecoveryRequired,
        ),
        (
            STAGING_DIRECTORY_FSYNC_FAILURE,
            "tx-staging-directory-fsync",
            QuarantineReplacementTransactionState::RecoveryRequired,
        ),
    ] {
