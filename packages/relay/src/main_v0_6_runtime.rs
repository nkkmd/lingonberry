...
fn route(
    ...
    match (method, path) {
        ...
        ("GET", route_path) if route_path.starts_with("/v1/effective-objects/") => {
            get_effective_object(
                route_path.trim_start_matches("/v1/effective-objects/"),
                backend,
            )
        }
        ("GET", route_path) if route_path.starts_with("/v1/objects/") => {
            get_object(route_path.trim_start_matches("/v1/objects/"), backend)
        }
        _ => Ok((
            404,
            "Not Found",
