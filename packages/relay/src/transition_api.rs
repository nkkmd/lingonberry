...
pub fn ingest_transition_request(
    ...
    let request = match parse_json(request_json) {
        Ok(value) => value,
        Err(error) => {
            return error_response(400, "Bad Request", "LB_INVALID_JSON", &error.to_string())
        }
    };
    let request_map = match as_object(&request) {
        ...
        None => {
            return error_response(
                400,
                "Bad Request",
                "LB_TRANSITION_ENVELOPE_INVALID",
                "transition request must be an object",
            )
        }
    };
    if request_map
        .keys()
        .any(|key| key != "transition" && key != "publisher")
    {
        return error_response(
            400,
            "Bad Request",
            "LB_TRANSITION_ENVELOPE_INVALID",
            "transition request contains unknown fields",
        );
    }
    ...
    if let Err(message) = verify_transition_signature(&request) {
        return error_response(
            401,
            "Unauthorized",
            "LB_TRANSITION_SIGNATURE_INVALID",
            &message,
        );
    }
    ...
    match find_existing(&paths.log, transition_id) {
        Ok(Some(existing)) if existing == canonical_request => {
            return success_response(
                ...
                transition_id,
                target_id,
                target_status(backend, target_id),
            );
        }
        Ok(Some(_)) => {
            return error_response(
                ...
                "transition id already exists with different immutable content",
            );
        }
        Ok(None) => {}
        Err(error) => {
    ...
    let record = JsonValue::Object(BTreeMap::from([
        (
            "storedAtUnixSeconds".to_string(),
            JsonValue::Number(stored_at),
        ),
        (
            "transitionId".to_string(),
            JsonValue::String(transition_id.to_string()),
        ),
        (
            "targetId".to_string(),
...
fn validate_publisher(publisher: &BTreeMap<String, JsonValue>) -> Result<(), String> {
    if publisher
        .keys()
        .any(|key| key != "publicKey" && key != "signature")
    {
        return Err("publisher contains unknown fields".to_string());
    }
    let public_key = string_field(publisher, "publicKey")
        .ok_or_else(|| "publisher.publicKey is required".to_string())?;
    let signature = string_field(publisher, "signature")
        .ok_or_else(|| "publisher.signature is required".to_string())?;
...
fn validate_transition(transition: &BTreeMap<String, JsonValue>) -> Result<(), String> {
    ...
    let target_id = string_field(transition, "targetId")
        .ok_or_else(|| "targetId must be a string".to_string())?;
    ...
    let transition_type = string_field(transition, "transitionType")
        .ok_or_else(|| "transitionType must be a string".to_string())?;
    match transition_type {
        "replace" => {
            let replacement_id = string_field(transition, "replacementId")
                .ok_or_else(|| "replace transition requires replacementId".to_string())?;
...
fn verify_transition_signature(request: &JsonValue) -> Result<(), String> {
    let map =
        as_object(request).ok_or_else(|| "transition request must be an object".to_string())?;
    let transition = map
        .get("transition")
        .ok_or_else(|| "missing transition".to_string())?;
    let publisher = map
        .get("publisher")
        .and_then(as_object)
        .ok_or_else(|| "missing publisher".to_string())?;
    let public_key_hex =
        string_field(publisher, "publicKey").ok_or_else(|| "missing public key".to_string())?;
    let signature_hex =
        string_field(publisher, "signature").ok_or_else(|| "missing signature".to_string())?;
    ...
    let payload = to_canonical_json(&JsonValue::Object(BTreeMap::from([
        (
            "publisher".to_string(),
            JsonValue::Object(unsigned_publisher),
        ),
        ("transition".to_string(), transition.clone()),
    ])));
    let public_key =
        decode_lower_hex(public_key_hex).ok_or_else(|| "invalid public key hex".to_string())?;
    let signature =
        decode_lower_hex(signature_hex).ok_or_else(|| "invalid signature hex".to_string())?;
...
fn verify_ed25519_with_openssl(
    ...
    let temp_root = std::env::temp_dir().join(format!(
        "lingonberry-transition-signature-{}",
        SystemTime::now()
...
fn find_existing(path: &Path, transition_id: &str) -> Result<Option<String>, String> {
    ...
    for line in BufReader::new(file).lines() {
        ...
        let Some(record_map) = as_object(&record) else {
            continue;
        };
    ...
    TransitionHttpResponse {
        ...
        body: JsonValue::Object(BTreeMap::from([
            ...
            (
                ...
                JsonValue::String(target_status.to_string()),
            ),
        ])),
    }
}
...
fn valid_rfc3339_shape(value: &str) -> bool {
    value.contains('T')
        && (value.ends_with('Z')
            || value.rfind('+').is_some()
            || value.rfind('-').is_some_and(|index| index > 9))
}
